...
from aiober.types import parse_object, ViberObject
...
from aiober.methods import SetWebhook
...
class Dispatcher(Router):
    ...
    async def set_webhook(self, url):
        return await self.bot.session.make_request(
            self.bot,
            method=SetWebhook(
                url=url,
                event_types=[
    ...
    async def _delay_task(self, coro, delay: int = 1):
    ...
    async def start_webhook(
        ...
        #asyncio.create_task(self._delay_task(self.set_webhook(domain+path)))
